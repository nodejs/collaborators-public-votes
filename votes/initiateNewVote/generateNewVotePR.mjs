...
for await (const member of readReadme(readLines({ input, crlfDelay }))) {
  (member.isTSC ? shareholders : allowedVoters).push(member);
}

input.destroy?.();

const shareholdersThreshold = 3; // Math.ceil(shareholders.length / 4);
...
await generateNewVoteFolder({
  ...
  shareholders: shareholders.map(({ email }) => email),
  allowedVoters: allowedVoters.map((voter) => `${voter.name} <${voter.email}>`),
  method: "Condorcet",
...
${
  "" && // let's not ping anyone yet
  allowedVoters
    .map(({ name, handle }) => `- ${name} @${handle} (Node.js collaborator)`)
    .join("\n")
}
